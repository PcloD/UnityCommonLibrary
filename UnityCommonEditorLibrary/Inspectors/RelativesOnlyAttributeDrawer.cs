...
namespace UnityCommonEditorLibrary.Inspectors
{
    ...
    public class RelativesOnlyAttributeDrawer : PropertyDrawer
    {
        ...
        {
            var propertyGameObject = (property.serializedObject.targetObject as Component)
                .gameObject;
            ...
            {
                return obj.transform.IsChildOf(propertyGameObject.transform) &&
                       obj != propertyGameObject;
            }
            if (_target.IsOnlyRuleSet(ValidRelatives.Parents))
            {
                return propertyGameObject.transform.IsChildOf(obj.transform) &&
                       obj != propertyGameObject;
            }

            // Then check multi-applicable rules
            if (_target.IsRuleSet(ValidRelatives.SameGameObject) &&
                obj == propertyGameObject)
            {
                return true;
            }
